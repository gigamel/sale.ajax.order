...
else
{
	...
	<script>
		...
		BX.Sale.OrderAjaxCustomComponent.init({
	...
	if ($arParams['SHOW_PICKUP_MAP'] === 'Y' || $arParams['SHOW_MAP_IN_PROPS'] === 'Y')
	{
		if ($arParams['PICKUP_MAP_TYPE'] === 'yandex')
		{
			...
			<script>
				(function bx_ymaps_waiter(){
					if (typeof ymaps !== 'undefined' && BX.Sale && BX.Sale.OrderAjaxCustomComponent)
						ymaps.ready(BX.proxy(BX.Sale.OrderAjaxCustomComponent.initMaps, BX.Sale.OrderAjaxCustomComponent));
					else
		...
		if ($arParams['PICKUP_MAP_TYPE'] === 'google')
		{
			...
			<script>
				...
				{
					if (BX.Sale && BX.Sale.OrderAjaxCustomComponent)
						BX.Sale.OrderAjaxCustomComponent.initMaps();
					else
		...
		<script>
			(function bx_counter_waiter(i){
				...
				if (typeof window['yaCounter<?=$arParams['YM_GOALS_COUNTER']?>'] !== 'undefined')
					BX.Sale.OrderAjaxCustomComponent.reachGoal('initialization');
				else
